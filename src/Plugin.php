...
class Plugin
{
	...
	public function __construct()
	{
		...
		$this->config->path =
			...
			'.php';
		$this->config->options = get_option('layartancap_config');
	...
	public function loadModels()
	{
		...
		foreach ($models as $model) {
			...
			foreach ($model->getHooks() as $hook) {
				$class = str_replace('LayarTancap\\WordPress\\Hook\\', '', get_class($hook));
	...
	private function loadHooks($dir)
	{
		...
		foreach ($controllers as $controller) {
			...
			$controller = '\\LayarTancap\\' . ucwords($dir) . '\\' . $name;
			...
			foreach ($controller->getHooks() as $hook) {
				...
				if (in_array(strtolower($class), $this->enableHooks)) {
					...
					$status = isset($this->config->options->layartancap_hooks->$key)
						? $this->config->options->layartancap_hooks->$key
	...
	public function setDefaultOption()
	{
		...
		if (empty($config) || !$config) {
			...
			update_option('layartancap_config', (object) $config);
